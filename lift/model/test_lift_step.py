import myhdl
from myhdl import block, always, instance, Signal, delay, StopSimulation
...
def top_lift_step(clk, dout, din,  data_in, toLift_Step, z,we_1, we, we_in,  addr, addr_in, muxsel_i, x, res_o, left_i, right_i, sam_i,flgs_i, update_i, update_o, rst_fsm, datactn_in, datactn, pc_data_in, pc_data_rdy , muxaddrsel, addr_left, addr_sam, addr_rht ):
	...
	return instances()
@block
def testbench(clk, dout, din, data_in, toLift_Step, z, we_1, we, we_in, addr_1, addr, addr_in, muxsel_i, x, res_o, left_i, right_i, sam_i, flgs_i, update_i, update_o, rst_fsm, datactn_in, datactn, pc_data_in, pc_data_rdy):
	instance_lift_step = lift_step(left_i, sam_i, right_i, flgs_i, update_i, clk, res_o, update_o)
	instance_ram = ram(dout, din, addr, we, clk) 
	...
	return instances()
tb = testbench(clk, dout, din, data_in, toLift_Step, z, we_1, we, we_in, addr_1, addr, addr_in, muxsel_i, x, res_o, left_i, right_i, sam_i, flgs_i, update_i, update_o, rst_fsm, datactn_in, datactn, pc_data_in, pc_data_rdy)
...
#toVerilog(top_lift_step, clk, dout, din, data_in, toLift_Step, z, we_1, we, we_in, addr, addr_in, muxsel_i, x, res_o, left_i, right_i, sam_i, flgs_i, update_i, update_o, rst_fsm, datactn_in, datactn, pc_data_in, pc_data_rdy, muxaddrsel, addr_left, addr_sam, addr_rht) 
