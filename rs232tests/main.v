...
always @(posedge iClk, negedge iRst) begin: MAIN_UPDATEBUFF0_0_RTL
    if ((iRst == 0)) begin
        iData <= 0;
        ldData <= 0;
        WriteEnable <= 0;
        rom_addr <= 0;
        obusy <= 0;
        updatebuff0_0_state1 <= 3'b000;
    end
    else begin
        updatebuff0_0_state1 <= 3'b000;
        case (updatebuff0_0_state1)
            3'b000: begin
                obusy <= 1;
                if (((!WriteEnable) && (!oWrBuffer_full))) begin
                    ldData <= rom_dout;
                    updatebuff0_0_state1 <= 3'b001;
                end
            end
            3'b001: begin
                updatebuff0_0_state1 <= 3'b010;
            end
            3'b010: begin
                iData <= ldData;
                WriteEnable <= 1;
                updatebuff0_0_state1 <= 3'b011;
            end
            3'b011: begin
                WriteEnable <= 0;
                if ((rom_addr < 11)) begin
                    rom_addr <= (rom_addr + 1);
                    updatebuff0_0_state1 <= 3'b100;
                end
                else begin
                    rom_addr <= 0;
                    updatebuff0_0_state1 <= 3'b000;
            ...
            3'b101: begin
                if ((ppscounter < 4774)) begin
                    ...
                    ppscounter <= 0;
                    updatebuff0_0_state1 <= 3'b110;
                end
            end
            default: begin
                if ((updatebuff0_0_state1 == 3'b110)) begin
                    obusy <= 0;
                    updatebuff0_0_state1 <= 3'b000;
                end
            end
