...
reg [7:0] pps0_0_sighi;
reg [2:0] updateecho0_state1;
...
always @(posedge iClk, negedge iRst) begin: MAINECHO_RS232PROGRAMMER0_IO_WRITE_SYNC
    if ((iRst == 0)) begin
...
always @(posedge iClk, negedge iRst) begin: MAINECHO_RS232_MODULE0_SEQ_LOGIC
    if ((iRst == 0)) begin
...
always @(RS232_Module0_write_addr, RS232_Module0_tx_addr) begin: MAINECHO_RS232_MODULE0_COMB_LOGIC
...
always @(posedge iClk) begin: MAINECHO_PPS0_0_PPSI
    if ((ppscounter < 50000000)) begin
        ppscounter <= (ppscounter + 1);
        if ((ppscounter < pps0_0_sighi)) begin
            sig <= 1;
        end
        else begin
            sig <= 0;
...
always @(posedge iClk, negedge iRst) begin: MAINECHO_UPDATEECHO0_RTL
    if ((iRst == 0)) begin
        iData <= 0;
        ldData <= 0;
        WriteEnable <= 0;
        rom_addr <= 0;
        obusy <= 0;
        updateecho0_state1 <= 3'b000;
    end
    else begin
        updateecho0_state1 <= 3'b000;
        case (updateecho0_state1)
            3'b000: begin
                if (((!WriteEnable) && (!oWrBuffer_full))) begin
                    if ((oData != 0)) begin
                        ldData <= oData;
                        obusy <= 1;
                        updateecho0_state1 <= 3'b001;
                    end
                    else begin
                        updateecho0_state1 <= 3'b000;
                    end
            ...
            3'b001: begin
                updateecho0_state1 <= 3'b010;
            end
            3'b010: begin
                iData <= ldData;
                WriteEnable <= 1;
                updateecho0_state1 <= 3'b011;
            end
            3'b011: begin
                WriteEnable <= 0;
                updateecho0_state1 <= 3'b100;
            end
            3'b100: begin
                WriteEnable <= 0;
                updateecho0_state1 <= 3'b101;
            end
            3'b101: begin
                if ((!sig)) begin
                    updateecho0_state1 <= 3'b101;
                    ...
                    updateecho0_state1 <= 3'b110;
                end
            end
            default: begin
                if ((updateecho0_state1 == 3'b110)) begin
                    obusy <= 0;
                    updateecho0_state1 <= 3'b000;
                end
            end
...
always @(posedge iClk) begin: MAINECHO_PWRUPRST0_LOGIC2
...
always @(rom_addr) begin: MAINECHO_ROM0_READ
    case (rom_addr)
        0: rom_dout = 72;
        1: rom_dout = 69;
        2: rom_dout = 76;
