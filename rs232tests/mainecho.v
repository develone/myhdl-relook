...
reg [2:0] updateecho0_lcl_Rx_addr;
...
always @(posedge iClk) begin: MAINECHO_PPS0_PPSI
...
always @(posedge iClk, negedge iRst) begin: MAINECHO_RS232_MODULE0_SEQ_LOGIC
    if ((iRst == 0)) begin
        ...
        oRx_addr <= RS232_Module0_rx_addr;
...
always @(posedge iClk, negedge iRst) begin: MAINECHO_UPDATEECHO0_RTL
    if ((iRst == 0)) begin
        ...
        case (updateecho0_state1)
            3'b000: begin
                if ((oRx_addr == updateecho0_lcl_Rx_addr)) begin
                    updateecho0_state1 <= 3'b000;
                end
                else begin
                    if (((!WriteEnable) && (!oWrBuffer_full))) begin
                        ldData <= oData;
                        updateecho0_lcl_Rx_addr <= oRx_addr;
                        obusy <= 1;
